...
const reservationColumns = [
  ...
  { 
    label: '고객명', 
    key: 'user',
    render: (item) => `<span class="cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400">${item.user}</span>`
  },
  { label: '연락처', key: 'phone' },
  { 
    label: '예약일시', 
  ...
  { 
    label: '상태', 
    key: 'status',
    render: (item) => `<span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(item.status)}">${getStatusText(item.status)}</span>`
  },
  { 
    label: '액션', 
    key: 'action',
    render: (item) => {
      ...
      const cancelBtn = item.status !== 'cancelled' 
