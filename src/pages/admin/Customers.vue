<template>
  <div class="space-y-6 p-6 text-gray-800 dark:bg-gray-900 dark:text-gray-200">
    ...
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">고객 관리</h1>
    ...
    />
     <!--@row-click="handleCustomerRowClick" 행 클릭 핸들러 : 고객 상세 보기 모달 열기 -->
    <!-- 삭제 확인 모달 -->
    <div
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
        ...
        <div class="p-6">
          <div class="space-y-6">
            ...
            <!-- 이용 정보 섹션 -->
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              <h5 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">이용 정보</h5>
              <div class="grid grid-cols-2 gap-4">
                <div>
            ...
            <!-- 선호 메뉴 섹션 -->
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              <h5 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">선호 메뉴</h5>
...
// 테이블 컬럼 정의 (고객 정보는 복잡하므로 간단하게 표시)
const customerColumns = [
    ...
    render: (item) => `
      <div class="flex items-center">
        <div class="relative">
          ...
          <span class="absolute bottom-0 right-0 block h-3 w-3 rounded-full ring-2 ring-white dark:ring-gray-800 ${getStatusDotClass(item.status)}"></span>
        </div>
        <div class="ml-4">
          <div class="text-sm font-medium text-gray-900 dark:text-white flex items-center">
            ${item.name}
            ${item.tags.includes('VIP') ? '<span class="ml-2 px-2 py-0.5 text-xs font-medium bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-300 rounded-full">VIP</span>' : ''}
          </div>
  ...
  { 
    ...
    key: 'status',
    render: (item) => `
      <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(item.status)}">${getStatusText(item.status)}</span>
      <div class="mt-2 flex flex-wrap gap-1">
        ${item.tags.map(tag => `<span class="px-2 py-0.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 rounded-full">${tag}</span>`).join('')}
      </div>
    `
  },
  { 
...
const customerFilterFn = (data, filters) => {
  console.log("filters", filters);//필터 옵션 값 왜 메개변수로 받아오는거지?    
  console.log("data", data);//고객 목록
